            ...
            try {
                ...
                if (!TextUtils.isEmpty(action)) {
                    if (Constants.ACTION_CMD.equals(action)) {
                        ...
                        // 切换信源
                        PowerKeyUtils.checkSource(Integer.valueOf(id));
                    } else if (Constants.ACTION_HDMI_HOME_CMD.equals(action)) {
                        String id = intentCmd.getStringExtra("id");
                        LogUtil.i(TAG, "ACTION_HDMI_HOME_CMD:id:" + id);
//                        ANWApplication.getSystemControlManager().setProperty("persist.inovel.autorun.pkg",id);
                        SharedPreferenceUtil.set("boot", id);
                    }
                }
    ...
    private void sendSimpleCMD(Intent intentCmd, int value) {
        ...
        if (!TextUtils.isEmpty(param)) {
            ...
            if (!TextUtils.isEmpty(state)) {
                ...
                if (!TextUtils.isEmpty(methodId) && methodId.equalsIgnoreCase("SetLightSource")) {
                    ...
                    if (temp == 0 || temp == 1 ) {
                        ...
                        int old = SharedPreferenceUtil.getInt("LightSource_user", 15);
                        mExplicit ="LightSource_user";
                        SharedPreferenceUtil.set("LightSource", 15);
                        SharedPreferenceUtil.set("LightSource_user", old);
                        SocketManager.instance().createSocket().setMethodId(methodId).setParameter(param, old).send();
                    }
                    LogUtil.i("SetLightSource: temp " + temp);

                } else if (!TextUtils.isEmpty(methodId) && methodId.equalsIgnoreCase("Switch3D")) {
                    Settings.System.putInt(ANWApplication.getApplication().getContentResolver(), Constants.KEY_3D_STATE,
                ...
                if (!TextUtils.isEmpty(methodId) && methodId.equalsIgnoreCase("SetColorCorrection")) {
                    ...
                    if (!TextUtils.isEmpty(methodId) && methodId.equalsIgnoreCase("SetLightSource")) {
                        value += 5;
                        SharedPreferenceUtil.set("LightSource_user", value);
                        LogUtil.i("SetLightSource: value " + value);
                    }
            ...
            } else if (methodId.equalsIgnoreCase("back")) {
                LogUtil.i("点击了取消键:");
                ServiceUtils.sendKeyCode(KeyEvent.KEYCODE_DPAD_LEFT);
    ...
    private void reset() {
        mCachedThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                SocketManager.instance().createSocket().setMethodId("SetLightSource").setParameter("value", 0).send();
                SharedPreferenceUtil.set("LightSource", 0);
...
//                mTvControlManager.FactorySetParamsDefault();
     ...
     * type 为2 ，代表视图全部为单选且是信源设置，value值为 6 或则 7，object 为空
     * type 为3 ，代表视图全部为3D设置，通过object 区分类型，object = “format” 表示3D状态，object = “running” 表示当前是否再播放视频
     * 梯形校正是例外，虽然它是进度条，但采用第一中方式传值
     * type 为4 ，代表视图全部为单选且是开机启动，value值为 0,1,2，object 为空
     */
                ...
                if (!TextUtils.isEmpty(param)) {
                    ...
                    switch (param) {
                        ...
                        case "reset":
                            ...
                            //标准模式 1080,    1024,   1024,   0,      0,       0
                            //暖色模式 1120,   1024,    1024,   0,      0,       0
                            //冷色模式 1080,    1024,   1064,   0,      0,       0
                            int[] redGain = {1080, 1120, 1080};
                            int[] buleGain = {1024, 1024, 1064};
                            int[] redOffset = {0, 0, 0};
                            int[] buleOffset = {0, 0, 0};
                            for (int i = 2; i >= 0; i--) {
                                ...
                                ret = mTvControlManager.FactoryWhiteBalanceSetRedOffset(0, i, redOffset[i]);
                                ret = mTvControlManager.FactoryWhiteBalanceSetGreenOffset(0, i, 0);
                                ret = mTvControlManager.FactoryWhiteBalanceSetBlueOffset(0, i, buleOffset[i]);
        ...
        mCachedThreadPool.execute(new Runnable() {
            ...
            public void run() {
                if (jsonObject.has("data")) {
                    ...
                    if (data.has("format")) {
                        int value = data.optInt("format");
//                        sendMsg(3, value, "format");
                        Settings.System.putInt(ANWApplication.getApplication().getContentResolver(), Constants.KEY_3D_STATE,
                                Integer.valueOf(value));
                        return;

                    } else if (data.has("running")) {
                        int value = data.optInt("running");
                        sendMsg(3, value, "running");
                        return;
                    }
    ...
    public void freshData(AiItem obj, Message mMessage) {
        mCachedThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    LogUtil.i(TAG, "freshData 消息类型 " + mMessage.arg1 + " 消息值 = " + mMessage.arg2 + " obj " + mMessage.obj);
                    if (mMessage.arg1 == 0) {
                        if (!TextUtils.isEmpty(CMDManager.getInstance().mExplicit) &&
                            ...
                            //光源设置获取值
                                ...
                                (CMDManager.getInstance().mExplicit.equalsIgnoreCase("LightSource"))) {
                            ...
                            for (int i = 0; i < obj.getSubs().size(); i++) {
                                ...
                                if (mMessage.arg2 > 1) {
                                    obj.getSubs().get(2).setValue(AiItem.ABLE);
                                }

                                LogUtil.i(TAG, "设置光源 = " + obj.getSubs().get(i).getValue());
                            }
                        } else {
                    ...
                    } else if (mMessage.arg1 == 3) {

                        if (String.valueOf(mMessage.obj).equalsIgnoreCase("format")) {
                            //3d状态的获取
                            for (int i = 0; i < obj.getSubs().size(); i++) {
                                obj.getSubs().get(i).setValue(AiItem.ABLE_NO);
                                ...
                                LogUtil.i(TAG, "set_3d_Value = " + obj.getSubs().get(i).getValue());
                            }
                            Settings.System.putInt(ANWApplication.getApplication().getContentResolver(), Constants.KEY_3D_STATE,
                                    Integer.valueOf(mMessage.arg2));

                        } else if (String.valueOf(mMessage.obj).equalsIgnoreCase("running")) {

                            //获取到当前是否在播放视频
                            if (mMessage.arg2 == 1) {
                                for (int i = 0; i < obj.getSubs().size(); i++) {
                                    obj.getSubs().get(i).getAttrs().setMulCheckable(AiItem.ABLE);

                                    LogUtil.i(TAG, "当前播放视频? " + obj.getSubs().get(i).getAttrs().getMulCheckable());
                                }
                            } else {
                                for (int i = 0; i < obj.getSubs().size(); i++) {
                                    obj.getSubs().get(i).getAttrs().setMulCheckable(AiItem.ABLE_NO);
                        ...
                        for (int i = 0; i < obj.getSubs().size(); i++) {
                            ...
                            LogUtil.i(TAG, "设置开机启动项 = " + obj.getSubs().get(i).getValue());
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
