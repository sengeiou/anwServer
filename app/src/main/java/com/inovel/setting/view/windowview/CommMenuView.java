...
public class CommMenuView extends ConstraintLayout implements OnEventTransfer<Message> {
    ...
    public static final int EVENT_REFRESH_MISSING_DATA = 0X300;//更新遗漏的数据
...
//                    mMenuViewHelper.showExitAnim(null);
            ...
            case EVENT_REFRESH:
                ...
                mNeedFresh = true;
                LogUtil.d(TAG, "EVENT_REFRESH 消息类型 " + msg.arg1 + " 消息值 = " + msg.arg2 + " obj " + msg.obj);
                mMessage.arg1 = msg.arg1;
                mMessage.arg2 = msg.arg2;
                mMessage.obj = msg.obj;
                if (mMessage.arg1 == 3) {
                    Message message = Message.obtain();
                    message.what = EVENT_REFRESH_MISSING_DATA;
                ...
                break;
            case EVENT_REFRESH_MISSING_DATA:

                if (mAiItem != null && mAiItem.getName().equalsIgnoreCase(XiriCommandService.SET3D)) {
                    LogUtil.d(TAG, "EVENT_REFRESH_MISSING_DATA 消息类型 " + mMessage.arg1 + " 消息值 = " +
                            mMessage.arg2 + " obj " + mMessage.obj);

                    CMDManager.getInstance().freshData(mAiItem, mMessage);
                    List<AiItem> list = mAdapter.getList();
                    list.clear();
                    list.addAll(mAiItem.getSubs());
...
//                    }
    ...
    private void send3dMessage(Message message) {
    ...
    private void pointTo(String name, boolean canBack) {
        ...
        if (item != null) {
            ...
            if (name.equalsIgnoreCase(XiriCommandService.SETSIGNAL)) {
                ...
                ViewHolderOperateImpl mHelper = new ViewHolderOperateImpl(getContext());
                mHelper.dispatchAction(Constants.CMD_GET_3D_STATE, -1);
    ...
    private AiItem updateValueIf3D(AiItem item) {
        if (item != null && item.getName().equalsIgnoreCase(XiriCommandService.SET3D)) {
...
//        LogUtil.d(TAG, " notifyFocusChanged " + obj);
    ...
    /**
     * 更新数据，当有获取的值时，需要设置数据后再更新
     *
     * @param obj 下一页数据
     */
    private void updatePage(@NonNull AiItem obj) {
        LogUtil.d(TAG, "updatePage mNeedFresh = " + mNeedFresh + " mCompleted " + mCompleted);
        mCompleted = false;
        AiItem aiItem = obj;

        mTitleView.setText(aiItem.getName());

        aiItem = updateValueIf3D(aiItem);
        if (mNeedFresh) {
            CMDManager.getInstance().freshData(aiItem, mMessage);
            mNeedFresh = false;
        }

        List<AiItem> list = mAdapter.getList();
        list.clear();
        list.addAll(aiItem.getSubs());
    ...
    private void init(Context context) {
        ...
        mMenuViewHelper = new CommMenuViewHelper(this, mHandler);
        /* 布局管理器 */
...
//                LogUtil.d(TAG, "RecyclerView   onAnimationStart   mCompleted " + mCompleted);
                //设置焦点View
...
//                LogUtil.d(TAG, " RecyclerView  onAnimationEnd    mCompleted " + mCompleted);
            }
...
//        LogUtil.d(TAG, "dispatchKeyEvent " + event.getAction() + ", keyCode =" + event.getKeyCode());
